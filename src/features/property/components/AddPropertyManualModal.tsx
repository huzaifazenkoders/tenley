...
import { Building2, ChevronRight, PhoneCall, Smartphone, Users } from "lucide-react";
...
const featureCards = [
  {
    ...
    description: "Automated after-hours emergency handling with intelligent routing",
  },
  {
    icon: <Building2 className="size-6 text-brand-primary-red-500" />,
    title: "Property Management",
    description: "Manage units, occupancy status, and tenant access in one place.",
  },
  {
  ...
  },
  {
    ...
    description: "Assign required on-site roles like technicians and property managers.",
  },
];
...
const AddPropertyManualModal = ({ open, onOpenChange }: Props) => {
  ...
  return (
    <Modal open={open} onOpenChange={onOpenChange} className="w-[694px] p-6 flex flex-col gap-6">
      {/* Header */}
      <div className="flex items-start justify-between">
        <div className="flex flex-col gap-1">
          ...
          <Dialog.Description className="text-brand-Text-500 text-sm font-normal leading-5">
            This will be the central place where emergencies, staff, and units are managed.
          </Dialog.Description>
        </div>
        <ModalClose />
      </div>
      ...
      {/* Banner image */}
      <Image
        src="https://placehold.co/682x193"
        alt="Property banner"
        width={682}
        height={193}
          ...
          <div key={i} className="flex items-center gap-5">
            {row.map(({ icon, title, description }) => (
              ...
              >
                ...
                <div className="flex flex-col gap-1">
                  <span className="text-brand-Text-950-d text-sm font-medium leading-5">{title}</span>
                  <span className="text-brand-Text-500 text-xs font-normal leading-4">{description}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="flex justify-end items-center gap-3">
        ...
        <Button onClick={() => { onOpenChange(false); router.push("/property/add-property/manual-property"); }}>
