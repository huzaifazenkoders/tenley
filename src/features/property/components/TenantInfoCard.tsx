  ...
  return (
    ...
    <div className="p-4 bg-brand-base-white rounded-xl outline outline-1 -outline-offset-1 outline-brand-Text-100 flex flex-col gap-5">
      <div className="flex items-center justify-between">
        <span className="text-brand-Text-950-d text-base font-semibold leading-5">{tenant.label}</span>
        <div className="flex items-center gap-3">
          ...
          <Dropdown
            items={[
              { value: "edit", label: "Edit Tenant", onClick: () => setEditOpen(true) },
              { value: "remove", label: <span className="text-brand-primary-red-600-d">Remove Tenant</span>, onClick: () => setEndOpen(true) },
            ]}
      ...
      <div className="flex flex-col gap-4">
        <InfoField icon={<User className="size-4 text-brand-Text-800" />} label="Tenant Name" value={tenant.name} />
        <div className="flex items-center justify-between">
          <InfoField icon={<Mail className="size-4 text-brand-Text-800" />} label="Email" value={tenant.email} />
          <div className="w-36">
            <InfoField icon={<Phone className="size-4 text-brand-Text-800" />} label="Phone Number" value={tenant.phone} />
          </div>
        </div>
      </div>
    </div>
    <AddTenantModal
      ...
      tenants={[]}
      prefill={{ name: tenant.name, email: tenant.email, phone: tenant.phone, role: tenant.role }}
      title="Edit Tenant"
    />
...
const MOCK_TENANTS: Tenant[] = [
  { id: "t1", label: "Tenant 01", name: "John Smith", email: "john@example.com", phone: "(555) 123-4567", role: "Head of Household" },
  { id: "t2", label: "Tenant 02", name: "Max William", email: "max@example.com", phone: "(555) 123-4567", role: "Family Member" },
  { id: "t3", label: "Tenant 03", name: "Alex Warren", email: "alex@example.com", phone: "(555) 123-4567", role: "Family Member" },
];
    ...
    <div className="p-6 bg-brand-base-white rounded-[20px] outline outline-1 -outline-offset-1 outline-brand-Text-100 flex flex-col gap-10 overflow-hidden">
      ...
      <div className="flex items-center justify-between">
        ...
        {hasTenants && (
          <Dropdown
            items={[
              { value: "add", label: "Add New Member", onClick: () => setModalOpen(true) },
              { value: "end", label: <span className="text-brand-primary-red-600-d">End Tenancy</span>, onClick: () => {} },
            ]}
      ...
      {hasTenants ? (
        ...
        <div className="flex flex-col items-center gap-4 py-6">
          ...
          <div className="flex flex-col items-center gap-1 text-center max-w-xs">
            <p className="text-brand-Text-950-d text-xl font-bold leading-6">No Tenants Added Yet</p>
      ...
      <AddTenantModal open={modalOpen} onOpenChange={setModalOpen} tenants={tenants} />
    </>
  );
};
