...
const RECORDS: EmergencyRecord[] = [
  { id: "1", summary: "AC not working in unit A2", dateReported: "03/05/2025", status: "In Progress", type: "Emergency", assignedStaff: ["https://placehold.co/32x32", "https://placehold.co/32x32"] },
  { id: "2", summary: "AC not working in unit A2", dateReported: "05/03/2025", status: "In Progress", type: "Emergency", assignedStaff: ["https://placehold.co/32x32", "https://placehold.co/32x32"] },
];
...
const EmergencyCard = ({ record }: { record: EmergencyRecord }) => {
  ...
  return (
    <div className="p-4 bg-brand-base-white rounded-xl outline outline-1 -outline-offset-1 outline-brand-Text-100 flex flex-col gap-3">
      ...
      <div className="flex items-center justify-between">
        <span className={cn("px-2.5 py-[3px] rounded-xl text-sm font-normal leading-5", badge.bg, badge.text)}>
          {record.status}
        </span>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center">
          {record.assignedStaff.map((src, i) => (
            <Image key={i} src={src} alt="staff" width={32} height={32} className="size-8 rounded-full border-2 border-white -ml-2 first:ml-0 object-cover" unoptimized />
          ))}
        </div>
      </div>
    </div>
...
const EmergenciesCard = () => {
  ...
  const [activeStatus, setActiveStatus] = useState<EmergencyStatus>("In Progress");
    ...
    <div className="flex-1 self-stretch p-6 bg-brand-base-white rounded-[20px] shadow-[0px_1px_10px_0px_rgba(0,0,0,0.08)] outline outline-1 -outline-offset-1 outline-brand-Text-100 flex flex-col gap-6 overflow-hidden">
        ...
        <div className="p-1 bg-zinc-100 rounded-full inline-flex w-full">
          {(["Emergency", "Work Orders"] as const).map((m) => (
            <button
              ...
              onClick={() => setMode(m)}
              className={cn(
                "flex-1 px-6 py-2 rounded-full text-xs font-medium leading-4 transition-all duration-200",
                mode === m
          ...
          {STATUS_TABS.map((tab) => (
            <button
              ...
              className={cn(
                ...
                activeStatus === tab ? "text-brand-primary-red-600-d" : "text-gray-500"
              )}
            >
              {tab}
              {activeStatus === tab && (
      ...
      <div className="flex flex-col gap-4">
        ...
        {RECORDS.filter((r) => r.status === activeStatus).length === 0 && (
          <p className="text-brand-Text-500 text-sm text-center py-8">No records found.</p>
        )}
      </div>
    </div>
  );
