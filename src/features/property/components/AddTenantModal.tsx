...
const validationSchema = Yup.object({
  ...
  tenant_type: Yup.string()
    .oneOf(Object.values(TenantType), "Tenant type is required")
...
const AddTenantModal = (props: Props) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting, resetForm }) => {
      if (isEdit) {
        setSubmitting(true);
        const { error } = await updateTenant(props.tenant.tenant_id, {
  ...
  const allEmails = new Set([
    ...
    ...pendingTenants.map((t) => t.email.toLowerCase())
  ]);
  const emailVal = formik.values.email.trim().toLowerCase();
  const isDuplicateEmail =
    !isEdit && emailVal.length > 0 && allEmails.has(emailVal);
  ...
  const TenantCard = ({
    ...
    t: Tenant | BulkCreateTenantPayload;
    index: number;
    isPending?: boolean;
  }) => {
    const tenantType =
      "tenant_type" in t ? t.tenant_type : (t as Tenant).tenant_type;
    const tenantName = "tenant_name" in t ? t.tenant_name : "";
    const badge = roleBadge[tenantType];
    return (
      <div
        className={cn(
          "flex-1 min-w-[280px] p-4 bg-brand-base-white rounded-xl outline outline-1 flex flex-col gap-5",
          isPending ? "outline-brand-primary-red-200" : "outline-brand-Text-100"
        )}
      >
        ...
        <div className="p-6 flex flex-col gap-5">
          ...
          <div className="flex flex-wrap gap-6">
            ...
            {pendingTenants.map((t, i) => (
              <TenantCard
                key={`pending-${i}`}
                t={t as unknown as Tenant}
                index={existingTenants.length + i}
                isPending
              />
            ))}
            <button
          ...
          <div className="flex items-center gap-6 justify-end w-full">
            ...
            <Button
              ...
              disabled={submitLoading || !pendingTenants.length}
            >
          ...
          <div className="p-6 flex flex-col gap-5">
            ...
            <div className="flex items-center justify-end w-full gap-6">
              ...
              <Button
                type="submit"
                size="lg"
                disabled={formik.isSubmitting || isDuplicateEmail}
              >
                {formik.isSubmitting
                  ? "Saving..."
                  : isEdit
