...
const staff = [
  { id: 1, name: "Alexander McGurk", role: "Maintenance Supervisor", status: "Invitation Sent" },
];
...
const AssignedStaffCard = () => (
  <div className="p-6 bg-brand-base-white rounded-[20px] shadow-[0px_1px_10px_0px_rgba(0,0,0,0.08)] outline outline-1 -outline-offset-1 outline-brand-Text-100 flex flex-col gap-6">
    ...
    <div className="flex flex-col gap-4">
      {staff.map((s) => (
        <div key={s.id} className="p-3 bg-stone-50 rounded-lg outline outline-1 -outline-offset-1 outline-brand-Text-100 flex items-center gap-2">
          <div className="size-11 bg-zinc-200 rounded-full flex items-center justify-center shrink-0">
            <User className="size-5 text-gray-600" />
          </div>
          <div className="flex-1 flex flex-col gap-1">
            <span className="text-brand-Text-950-d text-base font-semibold leading-5">{s.name}</span>
            <span className="text-brand-Text-500 text-xs font-normal leading-4">{s.role}</span>
          </div>
