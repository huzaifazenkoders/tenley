...
const PropertyInfoCard = () => (
  <div className="p-6 bg-brand-base-white rounded-[20px] outline outline-1 -outline-offset-1 outline-brand-Text-100 flex flex-col gap-10 overflow-hidden">
    ...
    <div className="flex items-center gap-3">
      {[1, 2, 3].map((i) => (
        <Image key={i} src="https://placehold.co/96x96" alt="property" width={96} height={96} className="rounded-xl object-cover" />
      ))}
    </div>

    <div className="flex flex-col gap-4">
      <div className="flex justify-between">
        <InfoField label="Property Address" value="123 Main Street Boulevard" />
        <InfoField label="Property Name" value="123 Main Street Boulevard" />
      </div>
      <div className="h-px bg-brand-Text-100" />
      <div className="flex justify-between">
        <InfoField label="City" value="Austin" />
